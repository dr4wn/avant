...
function module:createInstances(identifier)
	...
	for _, par in pairs(fixtureFolder:GetChildren()) do
		local parFixture = Fixture.new({
			personality = {
			...
			instances = {
				lens = par.tilt.Beam1,
				spotlight =  par.tilt.Beam1.SL,
				beam =  par.tilt.Beam1.light
			},
			extras = {
				brightnessMultiplier = 1
			},
		})
		FixtureController:Append(identifier, module.fixtureData.name, parFixture)
	end
