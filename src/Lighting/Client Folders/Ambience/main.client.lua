...
function module:createInstances(identifier)
	...
	for _, ambienceLight in pairs(fixtureFolder:GetChildren()) do
		local ambientFixture = Fixture.new({
			personality = {
				fixture = module.fixtureData.name,
				name = ambienceLight.Name,
				enabled = true
			},
			instances = {
				lens = ambienceLight:WaitForChild("LED"),
				spotlight = ambienceLight:WaitForChild("LED"):WaitForChild("SpotLight")
			},
			extras = {
				brightnessMultiplier = 1
			},
		})
		FixtureController:Append(identifier, module.fixtureData.name, ambientFixture)
	end
